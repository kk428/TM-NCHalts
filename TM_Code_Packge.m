...
(* ::Input:: *)
(*(* 2.2*)
(*	The halting probabilities of 3,2 Turing machines (TMs) could be caluculated in a similar manner, in theory, but it is computationally unreasonable. A potential set of functions for 3,2 TMs is included for completion.*)
(**)
...
(* ::Input:: *)
(*(* 2.3*)
(*	However, even randomly selecting halting 4,2 TMs is unfeasible, as it is too likely that only nonhalting machnines will be selected. Code included  regardless.*)
(**)*)
...
(*-1 in the HaltWhen function accounts for the fact that some TMs may "halt" from their initial position:they never changed from their initial state,and halt at step zero. Here n should represent the same n used in RepeatingTM,as it might not necessarily work if a smaller value is chosen. Although this function will produce a result for a non-halting function,it should not be used for that purpose.*)
...
(* ::Input:: *)
(*(* 3.4*)
(*	To evaluate the halting times of the entire range of TMs in the scope of this study, the following function was used. The values from ALLHALTING were used in both Tables 2 and 3 for the most common halting times and for the maximum halting times, respectively.*)
(**)*)
